...
void setup_ui(int nprocs, int delta, int row_start, int row_end, char* all_names, uint32_t* pixel_buffer, uint32_t* texture_buffer) {
    init_freetype();
    sf::RenderWindow window(sf::VideoMode({WIDTH, HEIGHT}), "Burning Ship Fractal - Multi-Mode Parallel");
#ifdef _WIN32
    ...
    while (running) {
        ...
        Config config = { max_iteraciones, x_min, x_max, y_min, y_max, running ? 1 : 0, (int)current_mode };
        ...
        if (current_mode == Mode::MPI) {
            ...
            int local_hist[16] = {0};
            burning_ship_mpi(x_min, x_max, y_min, y_max, row_start, row_end, WIDTH, HEIGHT, pixel_buffer, local_hist);
            ...
            int recv_hist[nprocs * 16];
            MPI_Gather(local_hist, 16, MPI_INT, recv_hist, 16, MPI_INT, 0, MPI_COMM_WORLD);

            std::memset(global_hist, 0, sizeof(global_hist));
            for (int i = 0; i < nprocs; ++i) {
                for (int b = 0; b < 16; ++b) {
                    global_hist[b] += recv_hist[i * 16 + b];
                }
            }
        } else {
            std::memset(global_hist, 0, sizeof(global_hist));
            if (current_mode == Mode::SERIAL) {
                burning_ship_serial(x_min, x_max, y_min, y_max, 0, HEIGHT, WIDTH, HEIGHT, texture_buffer, global_hist);
            } else if (current_mode == Mode::OMP) {
                burning_ship_omp(x_min, x_max, y_min, y_max, 0, HEIGHT, WIDTH, HEIGHT, texture_buffer, global_hist);
            } else if (current_mode == Mode::SIMD) {
                burning_ship_simd(x_min, x_max, y_min, y_max, 0, HEIGHT, WIDTH, HEIGHT, texture_buffer, global_hist);
        ...
        draw_text_to_texture((unsigned char*)texture_buffer, WIDTH, HEIGHT, iter_str.c_str(), 10, 90, 18);
        draw_text_to_texture((unsigned char*)texture_buffer, WIDTH, HEIGHT, dom_str.c_str(), 10, 120, 18);
        draw_text_to_texture((unsigned char*)texture_buffer, WIDTH, HEIGHT, hist_str.c_str(), 10, 150, 18);
        draw_text_to_texture((unsigned char*)texture_buffer, WIDTH, HEIGHT, fps_str.c_str(), 10, 180, 18);
